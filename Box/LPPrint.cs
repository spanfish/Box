...
namespace Box
{
    public delegate void PrintOpenDelegate(bool opened, string printName);
    ...
    class LPPrint
    {
        ...
        #region 连接LPT1打印机
        public async void OpenAsync(PrintOpenDelegate openDelegate)
        {           
        ...
        #region 打印内容
        public async void Write(String Mystring, PrintCompletedDelegate pcd)
        {
            Task<bool> printTask = Task.Run(() =>
            {
                return this.Write(Mystring);
            });
            await printTask;
            pcd();
        }
